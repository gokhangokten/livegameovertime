...
const PersonnelManager: React.FC<PersonnelManagerProps> = ({ employees, onAddEmployee, onDeleteEmployee }) => {
  ...
  const handleDelete = (id: string, name: string) => {
    if (confirm(`${name} isimli personeli silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.`)) {
      onDeleteEmployee(id);
    }
  };
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <h3 className="font-bold text-slate-900">Mevcut Personel Listesi</h3>
              ...
              {employees.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-6 py-12 text-center text-slate-400 italic text-sm">
                    Sistemde henüz personel bulunmamaktadır.
                  </td>
                </tr>
              ) : (
                [...employees].reverse().map(emp => (
                  <tr key={emp.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4">
                      ...
                      <div className="text-[10px] text-slate-400 font-mono uppercase">{emp.id}</div>
                    </td>
